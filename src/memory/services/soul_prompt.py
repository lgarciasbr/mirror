...
SELF_IDENTITY_UNAVAILABLE = "No user Self identity layer is available yet. Use only the base Self Voice grammar."
...
def compose_soul_self_voice_prompt(mem: MemoryClient) -> str:
    ...
    injected_identity = identity.strip() if isinstance(identity, str) and identity.strip() else SELF_IDENTITY_UNAVAILABLE
